...
**패키지:** `com.lisearcheleeds.lighthouse-extends.addressable`  
**의존 패키지:** UniTask · Unity Addressables

참조 카운트 방식의 Addressables 래퍼입니다. 스코프 단위의 에셋 수명 관리와 병렬 로드를 지원합니다. 주소별로 참조 카운트를 관리하며, 동일 주소를 여러 스코프가 공유할 경우 같은 `AsyncOperationHandle`을 재사용합니다. 마지막 핸들이 해제될 때 비로소 `Addressables.Release`가 호출됩니다.

### IAssetManager

DI에 `AssetManager`를 등록하고 `IAssetManager`로 주입받습니다. `CreateScope()`를 호출하면 에셋 로드용 `IAssetScope`를 얻을 수 있습니다.
...
씬마다(또는 논리적 로드 컨텍스트마다) 스코프를 하나씩 생성합니다. 스코프를 Dispose하면 취득한 모든 핸들이 일괄 해제됩니다.
...
| 메서드 | 설명 |
|---|---|
| `LoadAsync<T>(string address, ct)` | 주소로 단일 에셋 로드. `IAssetHandle<T>` 반환 |
| `LoadAsync<T>(IReadOnlyList<string> addresses, ct)` | 주소 리스트로 순차 로드. `IReadOnlyList<IAssetHandle<T>>` 반환 |
| `LoadByLabelAsync<T>(string label, ct)` | 라벨에 일치하는 모든 에셋 로드. `IReadOnlyList<T>` 반환 |
| `TryLoadAsync(ParallelLoadData data, ct)` | 다양한 타입의 에셋을 병렬 로드. 부분 실패 허용. `ParallelLoadResult` 반환 |
| `Dispose()` | 스코프가 취득한 모든 핸들 해제 |

### IAssetHandle\<T\>

| 멤버 | 설명 |
|---|---|
| `Asset` | 로드된 `UnityEngine.Object` |
| `IsDisposed` | `Dispose()` 호출 후 `true` |
| `Dispose()` | 참조 카운트 감소. 카운트가 0이 되면 `Addressables.Release` 호출 |

스코프 Dispose 이전에 개별 핸들을 먼저 Dispose하면 더 빨리 메모리를 해제할 수 있습니다.

### 병렬 로드

`TryLoadAsync`는 모든 로드를 동시에 시작하고 결과를 모아서 반환합니다. 하나의 요청이 실패해도 다른 요청은 취소되지 않습니다.
...
if (!result.IsSuccess(audioReq))
    Debug.LogWarning("BGM 로드 실패");
```

| 멤버 | 설명 |
|---|---|
| `ParallelLoadData.Add<T>(string address)` | 로드 요청 등록. `AssetRequest<T>` 토큰 반환 |
| `ParallelLoadResult.IsSuccess<T>(request)` | 요청 성공 여부 |
| `ParallelLoadResult.Get<T>(request)` | 성공 시 `IAssetHandle<T>`, 실패 시 `null` 반환 |

### 취소 동작

`CancellationToken`을 전달하면 **await**가 취소되지만, 해당 핸들을 다른 호출자가 공유하고 있을 수 있으므로 Addressables 내부 로드는 취소되지 않습니다. 토큰이 발동되면 `OperationCanceledException`이 발생하고 해당 주소의 참조 카운트가 감소합니다. 핸들은 스코프에 추가되지 않습니다.
